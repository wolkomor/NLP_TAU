...
def load_dataset(path, glove_dim, doc_length, SEED):
    ...
    if not Path(path).exists():
        df_data = pd.read_csv(path.replace("_tokenized", ""))
        df_data['text_tokenized'] = df_data['text'].apply(text_to_wordlist)
        # df_data[["word_count", "number_count", "average_word_length", "count_pos", "count_exclamation", "count_date"]] =\
        #     df_data['text'].apply(lexical_sentiment_features_extraction, result_type='expand')
        # df_data['categories'] = df_data['text'].apply(categories_extraction)
        # df_data['sentiment'] = df_data['text'].apply(sentiment_extraction)
        df_data.to_csv(path, index=False)

    #initialize glove embeddings
    TEXT = data.Field(sequential=True, tokenize=lambda x: x.spilt(" "), lower=False, include_lengths=True, batch_first=True,
