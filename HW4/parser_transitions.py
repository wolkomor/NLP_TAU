...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) != 0:
        ...
        for i, pars in enumerate(minibatch):
            if (len(pars.buffer)==0) and len(pars.stack)==1:
                unfinished_parses.remove(pars)
                continue
            pars.parse_step(transitions[i])


    for i, pars in enumerate(partial_parses):
        dependencies.append(pars.dependencies)
